...
all_fourth <- pbp %>%  filter(down == '4', qtr %in% c(1, 2, 3, 4))
...
home_coach_go <- go_for_it %>%
  ...
  summarise(count_home = n(), conversions = sum(fourth_down_converted))

away_coach_go <- go_for_it %>%
  rename('coach' = 'away_coach') %>% 
  group_by(coach) %>% 
  filter(posteam_type == 'away') %>%
  summarise(count_away = n(), conversions = sum(fourth_down_converted)) 
...
go_for_it_coach$converted = go_for_it_coach$conversions.x + go_for_it_coach$conversions.y
go_for_it_coach <- go_for_it_coach[, c(1, 6, 7)]
...
go_rates$conversion_rate <- round((go_rates$converted/go_rates$go) * 100, 2)
go_rates <- (go_rates[order(go_rates$go_percentage, decreasing = T), ]) 
go_rates <- go_rates[go_rates$all > 100, ]
go_rates <- mutate(go_rates, rank = row_number())

#### Plotting coach fourth down go percentages (min 100 4th downs) ####
plot_ly(x = go_rates$coach, y = go_rates$go_percentage, type="bar") %>% 
  layout(xaxis = list(categoryorder = "total descending"))

#### Plotting coach fourth down conversion artes (min 100 4th downs) ####
plot_ly(x = go_rates$coach, y = go_rates$conversion_rate, type="bar") %>% 
  layout(xaxis = list(categoryorder = "total descending"))
     
#### Fourth downs gone for by quarter ###
all_fourth %>% 
  mutate(Quarter = as.character(qtr)) %>% 
  group_by(Quarter) %>% 
  summarise(count = n()) %>% 
  plot_ly(x = ~Quarter, y=~count, type="bar")
